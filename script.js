...
{
    var today = moment();
    $("#currentDay").text(today.format("dddd, MMMM Do")); //currentday format
    ...
    for (var i = 9; i < 18; i++) {
      if (i < currentHour) {
        document.getElementById(i.toString()).classList.add('past'); //past time activities line
        } else if (i === currentHour) {
            document.getElementById(i.toString()).classList.add('present'); //present time activities line
        } else if (i > currentHour) {
            document.getElementById(i.toString()).classList.add('future'); //future time activities line
        }
    }
    //below codes are activities write and save lines.
    {
        {//   
